...
- **`cobol_executor.py`** — Compile-once/run-many executor. `prepare_context()` calls `incremental_instrument()` and compiles. `run_test_case()` writes mock data → executes COBOL → parses SPECTER-TRACE/@@B: output. `run_batch()` for parallel execution via ProcessPoolExecutor. Coverage mode sets `stop_on_exec_return=False`, `eib_calen=100`, `eib_aid=X'7D'`. When the caller supplies the AST program, `prepare_context` also builds the unified branch registry (see `branch_registry.py`), persists it as `branch_registry.json` in `work_dir`, and stores a Python-bid → COBOL-(bid, direction) translation map on the returned `CobolExecutionContext`. `ctx.translate_py_branch("py:<bid>:<dir>")` returns the matching COBOL probe label or `None` when no counterpart exists.

- **`branch_registry.py`** — Unified branch identity shared by `code_generator` (Python) and `cobol_mock._add_branch_tracing` (COBOL). `BranchEntry.content_hash` is `sha1(paragraph || normalized_condition || type || ordinal)`; `build_registry(program)` walks the AST in code-generation order so the N-th registry entry matches the N-th Python-side branch. The COBOL tracer stamps the same hash on its `branch_meta` (IF: `content_hash`; EVALUATE: `when_hashes[direction]` per WHEN arm, using the full multi-line WHEN condition), and `cobol_executor.prepare_context` joins the two by hash. COBOL-only branches (no AST anchor) reserve bids in the `COBOL_ONLY_BID_MIN = 90001+` range so they never collide with registry-assigned ids.
...
- **`memory_models.py`** / **`memory_store.py`** / **`persistence_utils.py`** / **`run_manifest.py`** — Memory-guided persistence layer. `MemoryState` aggregates `SuccessState`, `FailureFragment`, `TargetStatus`, `StrategyStats`, `APIBudgetLedger`. `MemoryStore` writes atomically (`atomic_write_text`, `atomic_write_json`, `append_line_with_fsync`) into a run-local directory derived from the test-store stem (e.g. `tests.jsonl` → `tests_memory/`). `run_manifest.py` tracks source hash, copybook roots, and phase checkpoints in `.specter_run_manifest.json`. Running `run_cobol_coverage` multiple times against the same test store accumulates coverage: each loop loads existing TCs, seeds mutation corpora from them, and appends new successes — so N sequential loops are a practical alternative to bumping `--coverage-budget` (fresh LLM sampling per loop often breaks through plateaus the previous loop left behind).

- **`supervisor_channel.py`** — Teacher/student IPC for long-running jobs. Env-gated via `SPECTER_SUPERVISOR=<run_dir>` (no-op when unset). `SupervisorChannel.escalate()` appends a JSON event to `<run_dir>/escalations.jsonl` and blocks polling `<run_dir>/resolutions.jsonl` for a UUID-correlated reply; `.heartbeat()` writes to `status.jsonl`. Valid verdicts: `patch` (inline `{line: content}` fix), `skip` (abandon this error), `abort`/`restart` (raise `SupervisorAbort`/`SupervisorRestart` which inherit from `BaseException` so `except Exception`/`except RuntimeError` cannot silently retry them). Replies may carry durable fields: `save_rule` → appended to `teacher_rules.jsonl` as a `TeacherRule` (phase + `msg_contains` + optional `source_context_contains`); `finding` → appended to `teacher_findings.jsonl` for later operator review; `save_fact` → appended to `teacher_facts.jsonl` as a `TeacherFact`. `TeacherRulesStore.match()` is consulted by `_compile_and_fix` and `_count_actionable_errors` so a taught skip rule applies on the next compile without another escalation round-trip. Escalation payloads include `related_findings` (prior findings + rules summary) so the teacher sees history. `scripts/teach.sh` is a `tail -F` helper for watching events from a teacher session.

- **Teacher facts (domain knowledge)**: `TeacherFact` + `TeacherFactsStore` (in `supervisor_channel.py`) store program-specific knowledge the student can't generalize in code — input formats, valid enumerations, stub semantics. Each fact carries `kind`, `target` (variable / paragraph / stub op key, uppercased), `scope` (`variable` | `paragraph` | `stub_op` | `global`), `content`, and optional `examples`. `cobol_coverage.run_cobol_coverage` loads `teacher_facts.jsonl` from the supervisor run dir and hands the store to `JITValueInferenceService`, which includes matching facts in its LLM prompt for every variable inference (and mixes a short digest into the cache key so new facts invalidate stale profiles). Facts are solicited at end-of-cycle via `_end_of_cycle_review()` — fires only when `SPECTER_SUPERVISOR` + `SPECTER_ESCALATE` are both set and there are uncovered branches. Policy: if the teacher can see a pattern that should be a code heuristic (e.g. any variable named `*-DATE` with PIC X(6)), use `finding` instead; facts are reserved for knowledge that is *specific to this program* and cannot be derived from names/types.

- **Teacher-supplied test cases**: `Resolution.test_cases` is an optional field on a supervisor-channel reply carrying concrete inputs the teacher validated in its own Python environment: `[{input_state, stub_log, target?, notes?}, ...]`. When `branch_swarm.py`'s exhaustion escalation comes back with test cases, the student runs each through `_execute_directly` (same path strategies use) — no teacher code runs on the student side, only data flows. Successful cases persist into the test store tagged as `target="direct:<para>|swarm:<branch>"`. Intended for stubborn branches where specialists can't find the right combo but the teacher figured it out off-line. Input-state keys get uppercased for consistency; `stub_log` accepts both list form (`[[op_key, [[var, val]]], ...]`) and dict form (`{op_key: [[[var, val]]]}`). See `tests/test_supervisor_persistence.py` for the reply-shape contract.
...
- **Teacher/student channel**: `SPECTER_SUPERVISOR=<run_dir>` + `--escalate` (sets `SPECTER_ESCALATE=1`) together enable the durable IPC loop — channel dir alone is not enough. Default is opt-out, so a mature student runs autonomously. On reviewer deadlock in `_compile_and_fix`, or after a stubborn branch exhausts all `branch_swarm` rounds, the student escalates to a watcher (typically a Claude Code session running `scripts/teach.sh`); verdicts are `patch`/`skip`/`abort`/`restart`/`retry_with`. Teacher `patch` replies are applied at error-count parity (not just strict decrease), `abort`/`restart` raise `BaseException` subclasses so they can't be retried away, and replies carrying `save_rule`/`finding` are persisted durably. If the teacher cannot help (skip / timeout / no reply) the student does not insist — it records the failure and moves on. Files written: `escalations.jsonl`, `resolutions.jsonl`, `status.jsonl`, `teacher_rules.jsonl`, `teacher_findings.jsonl`.
