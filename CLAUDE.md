...
- **`branch_agent.py`** — Inner agent loop for stubborn uncovered branches. When the coverage loop plateaus, picks the top-K highest-priority uncovered branches and runs a focused, multi-turn LLM investigation for each: gather context (backward slice + condition + domains + prior attempts) → LLM proposes input_state + stub_outcomes → execute → feed result back → loop up to `max_iterations` (default 3). Journals every attempt for cross-run persistence via `memory_state` and inclusion in the uncovered-branch report. Controlled by `--agent-iterations N`, `--no-branch-agent`, `SPECTER_BRANCH_AGENT=0`.
