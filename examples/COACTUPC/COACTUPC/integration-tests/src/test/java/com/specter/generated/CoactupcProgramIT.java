...
/**
 * Mockito spy-based integration tests for {@link CoactupcProgram}.
 *
...
class CoactupcProgramIT {
    ...
    static Stream<TestCaseData> testCases() throws IOException {
        InputStream is = CoactupcProgramIT.class.getResourceAsStream("/test_store.jsonl");
        if (is == null) {
            return Stream.empty();
        }
    ...
    void testWithMockitoSpy(TestCaseData tc) {
        ...
        if (!consumedKeys.isEmpty()) {
            verify(spyStubs, atLeastOnce()).applyStubOutcome(any(ProgramState.class), anyString());
        }
