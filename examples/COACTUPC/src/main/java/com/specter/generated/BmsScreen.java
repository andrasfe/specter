 ...
 */
public class BmsScreen implements TerminalScreen {

    private final Screen screen;
    private final TextGraphics graphics;
    private final List<TerminalScreen.Field> fields;
    private final Map<String, String> inputValues = new LinkedHashMap<>();
    private int activeFieldIndex = 0;

    public BmsScreen(List<TerminalScreen.Field> fields) throws IOException {
        this.fields = fields;
    ...
    public void sendMap(ProgramState state) throws IOException {
        ...
        for (TerminalScreen.Field field : fields) {
            if (field.type() == TerminalScreen.FieldType.INPUT && field.name().endsWith("I")) {
    ...
    public String waitForAction() throws IOException {
        List<TerminalScreen.Field> inputFields = fields.stream()
                .filter(f -> f.type() == TerminalScreen.FieldType.INPUT)
                .toList();
        ...
        while (true) {
            ...
            if (key.getKeyType() == KeyType.Character) {
                TerminalScreen.Field f = inputFields.get(activeFieldIndex);
    ...
    private void positionCursor(TerminalScreen.Field field, int offset) {
    ...
    private void redrawInputField(TerminalScreen.Field field, String value) {
        String display = field.masked()
