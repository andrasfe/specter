 ...
 */
public class Section0_COACTUPC extends SectionBase {

    public Section0_COACTUPC(ParagraphRegistry registry, StubExecutor stubs) {
    ...
    void do_0000_MAIN(ProgramState state) {
        ...
        else if (((CobolRuntime.isTruthy(state.get("ACUP-DETAILS-NOT-FETCHED"))) && (CobolRuntime.isTruthy(state.get("CDEMO-PGM-ENTER")))) || ((java.util.Objects.equals(state.get("CDEMO-FROM-PROGRAM"), state.get("LIT-MENUPGM"))) && (!(CobolRuntime.isTruthy(state.get("CDEMO-PGM-REENTER")))))) {
            state.addBranch(7);
        ...
        else if ((CobolRuntime.isTruthy(state.get("ACUP-CHANGES-OKAYED-AND-DONE"))) || (CobolRuntime.isTruthy(state.get("ACUP-CHANGES-FAILED")))) {
            state.addBranch(8);
            state.put("WS-THIS-PROGCOMMAREA", state.get("WS-THIS-PROGCOMMAREA") instanceof Number ? 0 : "");
            state.put("CDEMO-PGM-REENTER", false);
            state.put("CDEMO-PGM-ENTER", true);
