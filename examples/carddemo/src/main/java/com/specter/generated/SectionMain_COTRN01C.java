 ...
 */
public class SectionMain_COTRN01C extends SectionBase {

    public SectionMain_COTRN01C(ParagraphRegistry registry, StubExecutor stubs) {
        ...
        paragraph("SEND-TRNVIEW-SCREEN", this::do_SEND_TRNVIEW_SCREEN);
        paragraph("RECEIVE-TRNVIEW-SCREEN", this::do_RECEIVE_TRNVIEW_SCREEN);
        paragraph("POPULATE-HEADER-INFO", this::do_POPULATE_HEADER_INFO);
        paragraph("READ-TRANSACT-FILE", this::do_READ_TRANSACT_FILE);
    ...
    void do_MAIN_PARA(ProgramState state) {
        ...
        if (java.util.Objects.equals(state.get("EIBCALEN"), 0)) {
            ...
            if (!(CobolRuntime.isTruthy(state.get("CDEMO-PGM-REENTER")))) {
                ...
                if (!java.util.List.of(" ", "\u0000").contains(state.get("CDEMO-CT01-TRN-SELECTED"))) {
                    state.addBranch(3);
                    state.put("TRNIDINI", state.get("CDEMO-CT01-TRN-SELECTED"));
                    perform(state, "PROCESS-ENTER-KEY");
                } else {
                    state.addBranch(-3);
                }
                perform(state, "SEND-TRNVIEW-SCREEN");
            } else {
                state.addBranch(-2);
                perform(state, "RECEIVE-TRNVIEW-SCREEN");
                ...
                if ((java.util.Objects.equals(_evalSubject1, "DFHENTER"))) {
                    ...
                    state.addBranch(8);
                    state.put("CDEMO-TO-PROGRAM", "COTRN00C");
                    perform(state, "RETURN-TO-PREV-SCREEN");
                }
                else {
                    state.addBranch(9);
    ...
    void do_PROCESS_ENTER_KEY(ProgramState state) {
        if ((java.util.List.of(" ", "\u0000").contains(state.get("TRNIDINI")))) {
            state.addBranch(10);
            state.put("WS-ERR-FLG", "Y");
            state.put("WS-MESSAGE", "Tran ID can NOT be empty...");
            state.put("TRNIDINL", -1);
            perform(state, "SEND-TRNVIEW-SCREEN");
        }
        else {
        ...
        if (!(CobolRuntime.isTruthy(state.get("ERR-FLG-ON")))) {
            ...
            perform(state, "READ-TRANSACT-FILE");
        } else {
            state.addBranch(-12);
        }
        if (!(CobolRuntime.isTruthy(state.get("ERR-FLG-ON")))) {
    ...
    void do_SEND_TRNVIEW_SCREEN(ProgramState state) {
        perform(state, "POPULATE-HEADER-INFO");
        state.put("ERRMSGO", state.get("WS-MESSAGE"));
        stubs.dummyExec(state, "CICS", "EXEC CICS SEND MAP('COTRN1A') MAPSET('COTRN01') FROM(COTRN1AO) ERASE CURSOR END-EXEC");
    }

    void do_RECEIVE_TRNVIEW_SCREEN(ProgramState state) {
        stubs.dummyExec(state, "CICS", "EXEC CICS RECEIVE MAP('COTRN1A') MAPSET('COTRN01') INTO(COTRN1AI) RESP(WS-RESP-CD) RESP2(WS-REAS-CD) END-EXEC");
    }
    ...
    void do_READ_TRANSACT_FILE(ProgramState state) {
