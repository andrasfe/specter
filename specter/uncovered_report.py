...
class UncoveredReport:
    ...
    compile_fix_overrides: list[dict[str, Any]] = field(default_factory=list)
...
def _extract_condition_vars(text: str) -> list[str]:
    ...
    Quoted literals are stripped first so that identifier-shaped tokens
    inside strings (e.g. ``'ERRORES'``, ``'SET:'``) are not returned
    as variable names.
    """
    if not text:
        return []
    # Remove quoted literals so their contents aren't misidentified as vars.
    stripped = _QUOTED_LITERAL_RE.sub("", text)
    seen: set[str] = set()
    out: list[str] = []
    for match in _VAR_RE.finditer(stripped):
...
def _generate_uncovered_report_impl(
    *,
    ...
    memory_targets = getattr(getattr(ctx, "memory_state", None), "targets", None) or {}
    ...
    for bid_key, meta in branch_meta.items():
        ...
        for direction in ("T", "F"):
            ...
            if paragraph and paragraph in runtime_only_paragraphs:
                continue
            # Infrastructure paragraphs generated by the mock pipeline
            # (SPECTER-READ-INIT-VARS, SPECTER-APPLY-MOCK-PAYLOAD, etc.)
            # are not business logic.  They may not be in
            # runtime_only_paragraphs when no test case reached them.
            if paragraph and (
                paragraph.startswith("SPECTER-")
                or paragraph == "FILE-CONTROL"
            ):
                continue
            ...
            # Branches whose condition only references mock-infrastructure
            # variables (MOCK-EOF-FLAG, MOCK-ALPHA-STATUS, …) are injected
            # by the I/O/CALL/EXEC replacement phases — not business logic.
            if cond_vars and all(v.startswith("MOCK-") for v in cond_vars):
                continue
            ...
            mem_total, mem_branch, mem_para = _count_attempts_from_memory_targets(
                memory_targets,
                bid,
                direction,
                paragraph,
            ...
            if mem_total > total:
                ...
                attempts = [
                    AttemptSummary(
                        strategy="memory_target_status",
                        count=mem_total,
                        direct_bid_match=mem_branch,
                        paragraph_match=mem_para,
                    )
                ]
    ...
    # --- load reviewer overrides from compile-time log ---
    compile_overrides: list[dict[str, Any]] = []
    if mock_source_path:
        overrides_path = Path(mock_source_path).parent / "reviewer_overrides.jsonl"
        if overrides_path.exists():
            try:
                for raw_line in overrides_path.read_text(errors="replace").splitlines():
                    raw_line = raw_line.strip()
                    if raw_line:
                        compile_overrides.append(json.loads(raw_line))
            except Exception as exc:  # noqa: BLE001
                log.debug("Could not load reviewer overrides: %s", exc)
    ...
    uncovered_report = UncoveredReport(
        ...
        compile_fix_overrides=compile_overrides,
    )
...
def _render_markdown(report: UncoveredReport) -> str:
    ...
    if report.compile_fix_overrides:
        lines.append("## Compile-fix reviewer overrides")
        lines.append("")
        lines.append(
            "Fixes abandoned or bypassed after the LLM challenger "
            "rejected the scribe's proposals."
        )
        lines.append("")
        for ov in report.compile_fix_overrides:
            outcome = ov.get("outcome", "unknown")
            phase = ov.get("phase", "?")
            line = ov.get("line", "?")
            revisions = ov.get("revisions_attempted", 0)
            lines.append(
                f"- **Line {line}** (phase `{phase}`, "
                f"{revisions} revision(s), outcome: `{outcome}`)"
            )
            err = ov.get("error", "")
            if err:
                lines.append(f"    - Error: `{err[:120]}`")
            fix_sum = ov.get("fix_summary", "")
            if fix_sum:
                lines.append(f"    - Fix: {fix_sum[:120]}")
            reason = ov.get("reviewer_reason", "")
            if reason:
...
def _count_attempts_from_memory_targets(
    memory_targets: dict[str, Any] | Any,
    bid: int,
    direction: str,
    paragraph: str,
) -> tuple[int, int, int]:
    """Approximate attempts using persisted memory target counters."""
    if not memory_targets:
        return 0, 0, 0

    branch_key = f"branch:{bid}:{str(direction or '').upper()}".lower()
    want_para = str(paragraph or "").upper()

    total = 0
    branch_total = 0
    para_total = 0

    for raw_key, raw_status in dict(memory_targets).items():
        key = str(raw_key or "").strip()
        if not key:
            continue

        if isinstance(raw_status, dict):
            attempts = int(raw_status.get("attempts", 0) or 0)
        else:
            attempts = int(getattr(raw_status, "attempts", 0) or 0)
        if attempts <= 0:
            continue

        key_l = key.lower()
        if key_l == branch_key:
            branch_total += attempts
            total += attempts
            continue

        if want_para and key_l.startswith("para:"):
            para_name = key[5:].strip().upper()
            if para_name == want_para:
                para_total += attempts
                total += attempts

    return total, branch_total, para_total
